...
| **Search & Addresses** | `LIT/`, `LIT3GP/` (`EJ211*.db`) | OSM `addr:*` + Nominatim centroids | `Geographic.gdb` SQLite builder + native `LitCompiler` producing 544B physical FLDB pages with rotary speller prefix Trie | **RESOLVED** | Implemented in `mmi-formats::hb_lit` and `mmi-rebuild::lit_compiler` per ADR-010. |
| **3D City & Terrain Models**| `CTY/`, `TER/` (`*.ATLAS`, ~11.5 GiB)| NASA SRTM / OSM 3D buildings | Full `AtlasCompiler` generating Orion ATLAS spatial tile quad-tree with Morton keys, 65×65 terrain elevation grids, and OSM building meshes | **RESOLVED** | Implemented in `mmi-formats::hb_atlas` and `mmi-rebuild::atlas_compiler` per ADR-011. |
